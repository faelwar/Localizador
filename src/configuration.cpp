...
void Configuration::setFilePath(QString file){
    fullFilePath = file;
}

QString Configuration::getFilePath(){
    return fullFilePath;
}
...
bool Configuration::isAP(AccessPoint ap){
    for(int i = 0; i < apList.size(); i++){
        if(apList[i]->getMAC() == ap.getMAC()){
            return true;
        }
    }
    return false;
