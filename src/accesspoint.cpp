...
QString AccessPoint::getMAC() const {return mac;}
...
void AccessPoint::setguiX(int x){logX = guiX = x;}
void AccessPoint::setguiY(int y){logY = guiY = y;}

void AccessPoint::writeToFile(QXmlStreamWriter* xmlWriter){
    cout << ESSID.toStdString() << ";" << mac.toStdString() << endl;
    xmlWriter->writeStartElement("ACCESSPOINT");
        xmlWriter->writeTextElement("MACAddress", mac);
        xmlWriter->writeTextElement("ESSID", ESSID);
